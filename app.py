...
def parse_start_time_to_local(start_time_str):
    ...
    try:
        ...
        return start_time_str[11:16] if len(start_time_str) >= 16 else start_time_str
...
def prettify_outcome_label(raw_label, home, away, index=0, market_name=""):
    ...
    if any(x in lower for x in ["1x2", "full time result", "match result"]):
        if index == 0:
            return f"1 ({home})"
        if index == 1:
            return "X (Empate)"
    ...
    if any(x in lower for x in ["both teams to score", "btts"]):
        if index == 0:
            return "Sim"
        if index == 1:
            return "Não"

    if "draw no bet" in lower:
        if index == 0:
            return home
        if index == 1:
            return away
...
def classify_market(name):
    ...
    if any(x in n for x in ["both teams to score", "btts"]):
        return "btts"
    if any(x in n for x in ["over/under", "total goals", "totals", "goal line"]):
        return "totals"
    return "other"


def sort_markets(markets):
    priority = {"main": 0, "totals": 1, "btts": 2, "dnb": 3, "other": 9}
    return sorted(markets, key=lambda m: priority.get(m.get("type", "other"), 9))
...
def build_match(fixture):
    ...
    home = fixture.get("participant1Name", "Time 1")
    away = fixture.get("participant2Name", "Time 2")
    ...
    for market_id, market_data in markets.items():
        ...
        for idx, row in enumerate(rows):
            pretty_label = prettify_outcome_label(
                row["label"], home, away, idx, market_name
            )
            pretty_rows.append((pretty_label, row["price"]))
        ...
        if market_type == "main" and not main_odds:
    ...
    return {
        ...
        "time": parse_start_time_to_local(fixture.get("startTime", "")),
...
def fetch_today_matches():
    ...
    unique = {}
    for m in matches:
        key = f"{m['home']}|{m['away']}|{m['league']}|{m['time']}"
        unique[key] = m

    final_matches = list(unique.values())
    final_matches.sort(key=lambda x: x["confidence_percent"], reverse=True)
    CACHE["matches"] = {"ts": time.time(), "data": final_matches}
...
def format_main_odds(match):
    ...
    if len(odds) >= 3:
        ...
        b = odds[1][1]
        c = odds[2][1]
        return f"1: {a} | X: {b} | 2: {c}"

    return " | ".join(f"{label}: {price}" for label, price in odds)
